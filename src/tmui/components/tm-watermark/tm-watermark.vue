...
import { qr } from "../tm-qrcode/drawing.js"
// #endif
...
const imageWaterStyle = computed(() => {
  return {
    ...
    backgroundPosition: props.cross
      ? `${
          props.unit == "rpx" ? uni.upx2px(props.width / 2) : props.width / 2
        }px ${
...
async function init() {
  ...
  const canvasWidthPx =
    props.unit == "rpx" ? uni.upx2px(canvasWidth) : canvasWidth
  const canvasHeightPx =
    props.unit == "rpx" ? uni.upx2px(canvasHeight) : canvasHeight
  ...
  if (ctx) {
    ctx.translate(0, 0)
    const markWidth =
      props.unit == "rpx" ? uni.upx2px(width * ratio) : width * ratio
    const markHeight =
      props.unit == "rpx" ? uni.upx2px(height * ratio) : height * ratio
    if (debug) {
