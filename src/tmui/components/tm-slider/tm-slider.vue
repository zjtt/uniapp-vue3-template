<template>
  ...
  >
    <view
      ...
      :style="[
        {
          width: _sliderBarCssWidth + 'rpx',
          height: _sliderBarCssHeight + 'rpx'
        }
      ]"
    >
      <tm-sheet
        ...
        :color="props.bgColor"
        :height="
          props.direction == 'horizontal' ? props.height : _sliderBarCssHeight
        "
        :width="
          props.direction == 'horizontal' ? _sliderBarCssWidth : props.height
        "
        :margin="[0, 0]"
        :padding="[0, 0]"
      ></tm-sheet>
    ...
    <view
      ...
      :class="[
        props.direction == 'horizontal'
          ? 'flex-col flex-col-start-center'
          : ' flex-row '
      ]"
      :style="[
        props.direction == 'horizontal'
      ...
      <view
        ...
        :style="[
          !isNvue
            ...
            : {
                left: `${nvueDetailPos.left}px`,
                top: `${nvueDetailPos.bottom}px`
              }
        ]"
      >
        <tm-sheet
...
watch(
  ...
  () => {
    if (!isDablue.value) {
      ...
      btnPos.value[0].x = Math.abs(
        (Number(props.modelValue[0]) / _valueMax.value) *
          uni.upx2px(props.width)
      )
      btnPos.value[1].x = Math.abs(
        (Number(props.modelValue[1]) / _valueMax.value) *
          uni.upx2px(props.width)
      )
    }
  }
...
function getValue() {
  if (!isDablue.value) {
    return Math.ceil(
      (Math.abs(btnPos.value[0].x) / uni.upx2px(props.width)) *
        _valueMax.value +
        props.min
    )
  } else {
    return [
      ...
      Math.ceil(
        (Math.abs(btnPos.value[1].x) / uni.upx2px(props.width)) *
          _valueMax.value +
          props.min
      )
    ]
